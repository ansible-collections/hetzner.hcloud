...
options:
    ...
    include_architecture_wildcard:
        description:
            - Include ISOs with wildcard architecture (architecture is null). Works only if architecture filter is specified.
        type: bool
...
hcloud_iso_info:
    ...
    contains:
        ...
        architecture:
            description: Type of cpu architecture this ISO is compatible with. None indicates no restriction on the architecture (wildcard).
            returned: always
            type: str, None
            sample: x86
        type:
    ...
    def get_iso_infos(self):
        try:
            if self.module.params.get("id") is not None:
                self.hcloud_iso_info = [self.client.isos.get_by_id(
                    self.module.params.get("id")
                )]
            elif self.module.params.get("name") is not None:
                self.hcloud_iso_info = [self.client.isos.get_by_name(
...
def main():
    ...
    ansible_info = {
        'hcloud_iso_info': result['hcloud_iso_info']
    }
